...
/// Learner Portal 1-year price chart, matching the main portal layout
/// (axes, DMA overlays, avg-buy line, buy/sell/hold markers, zoom, hover).
class LearnerOneYearChart extends StatefulWidget {
  ...
  @override
  State<LearnerOneYearChart> createState() => _LearnerOneYearChartState();
}

class _LearnerOneYearChartState extends State<LearnerOneYearChart> {
    ...
    return Column(
      ...
      children: [
        ...
        if (widget.error != null && widget.history.isEmpty)
          ...
          _buildChart(widget.compact),
      ],
    );
  }

  Widget _buildChart(bool compact) {
    final prices = widget.history
        .map((e) => (e['price'] as num?)?.toDouble() ?? 0)
        .toList();
    final avgBuyPrice = widget.holding?.buyPrice ?? 0;
    final ma7 = _dmaLevel(prices, 7, widget.trend?.ma7);
    final ma20 = _dmaLevel(prices, 20, widget.trend?.ma20);
    final ma50 = _dmaLevel(prices, 50, widget.trend?.ma50);
    final markers = _actionMarkers(widget.holding);
    ...
    final maxX = (widget.history.length - 1).toDouble();
    ...
    final lineBars = <LineChartBarData>[
      ...
      if (ma7 != null)
        _dmaLevelBar(ma7, _ma7Color, labelFor(_ma7Color), compact: compact),
      if (ma20 != null)
        _dmaLevelBar(ma20, _ma20Color, labelFor(_ma20Color), compact: compact),
      if (ma50 != null)
        _dmaLevelBar(ma50, _ma50Color, labelFor(_ma50Color), compact: compact),
      if (avgBuyPrice > 0)
        LineChartBarData(
                              ...
                              sideTitles: SideTitles(
                                ...
                                getTitlesWidget: (value, meta) {
                                  ...
                                  if (index >= 0 &&
                                    ...
                                    final dateStr =
                                        widget.history[index]['date'] as String;
  ...
  List<_ActionMarker> _actionMarkers(Stock? holding) {
    if (holding == null || widget.history.isEmpty) return [];
  ...
  int? _indexForDateInRange(DateTime date) {
    ...
    final last = _historyDay(widget.history.length - 1);
  ...
  DateTime? _historyDay(int index) {
    ...
    final raw = widget.history[index]['date']?.toString();
